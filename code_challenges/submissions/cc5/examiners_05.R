...
## b. Display the first 5 records. Name it `head_toothgrowth`. 

# head(______, _)
head_toothgrowth = head(ToothGrowth, 5)
...
write.table(ToothGrowth, "data/ToothGrowth_written", 
            append = FALSE, quote=FALSE, row.names = FALSE)
# PASS
...
penguin_data = read.csv("data/penguins_lter.csv")
...
selected_features = select(penguin_data,
...
Adelie_data = filter(penguin_data,
...
ordered_penguin_data = arrange(penguin_data,
                               desc(Flipper.Length..mm.))
head(ordered_penguin_data)
...
modified_penguin_data = mutate(penguin_data, 
                              Flipper.Length..cm. = Flipper.Length..mm./10) 
head(modified_penguin_data, 10)
...
penguin_data_by_island = group_by(penguin_data, 
                                  Island) %>% # the column to be grouped by
                                  tally()

head(penguin_data_by_island, 10)
...
final_penguin_data = select(penguin_data, # insert name of the original penguin dataframe
