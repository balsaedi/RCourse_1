...
```{r ex="activity-2b", type="solution"}
# I. Find the data type of any whole number using `class()` function. **Remember to add `L` after the digits**
...
```{r ex="activity-2c", type="solution"}
# I. Find the data type of the value `"school"
class("school")
...
```{r ex="activity-2e", type="solution"}
#i. 3i + 8
class(3i + 8)
...
```{r ex="activity-2f", type="solution"}
# i. "Vipin"
charToRaw("Vipin")
...
```{r ex="activity-2g", type="solution"}
# vector ages with elements 21, 32, 22, 24, 27, 54, 20, 13
...
```{r ex="activity-2h", type="solution"}
#Create a vector named `height` with its elements/values as `120.1, 118, 123.4, 130.8, 115.2` and do the following;
...
```{r ex="activity-2i", type="solution"}
# Create a vector named `ages` and insert the following values; `13, 59, 27, 22, 19, 31, 43`
...
```{r ex="activity-2j", type="solution"}
# Create vector weights from 67, 55, 60, 59, 57.2, 71, 62, 66, 70
...
```{r ex="activity-2k", type="solution"}
# Create vector1 and vector2 as instructed 
vector1 = c(4, 6, 12, 7)
vector2 = c(7, 3, 5, 10)
...
2. Create a vector, `my_vector` with the following values; `12, 54, 76, 71, 42, 75, 33` and perform the following operations; 
    ...
    b. This part will require you to access and manipulate the vector elements of `my_vector`. 
        ...
        ii. Subset the vector from the second to the fifth element and print it to the console. Name it `my_vector_1`.
...
3. Create two vectors; `vector1` and `vector2`that have the elements `12, 76, 45, 23, 64, 88` and `22, 87, 10, 5, 99, 54` respectively.Use the two vectors to answer the following questions. 
    ...
    a. Use `summarise()` function to find the descriptive statistics of`vector1`. Name it `summary1` 
    ...
    e. Find the division of the `vector1` and `vector2`. Name it `vector6`.
