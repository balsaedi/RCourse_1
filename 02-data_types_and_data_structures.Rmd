...
These represent numeric values such as integers and decimals. They are used for mathematical expressions and quantitative data analysis. Example:
```{r}
a=23.5
class(a) #check the data type of a 
```

a whole number without without a decimal is also numeric for instance 45, 8, 0 and 73. 
```{r}
class(45)
```
...
"Hello world" can be represented as 
```{r}
